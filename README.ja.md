...
INSERT, UPDATE, DELETE 操作をリアルタイムで監視。複数のテーブルを同時に監視し、色分けで変更を表示します。

![リアルタイム監視](demo/1_RealTimeTableWaching.gif)

### イベントタイムライン

変更履歴を時系列で表示。相関グルーピングで関連する変更をまとめて確認できます。

![イベントタイムライン](demo/2_EventTimeline.gif)

### ERD 可視化

インタラクティブな ER 図。ドラッグ＆ドロップでテーブル配置、外部キーのリレーション表示、スキーマフィルタリングに対応。

![ERD 可視化](demo/3_ERDiagram.gif)

### Dry Run モード

SQL を安全にテスト。自動ロールバックにより、DB を変更せずに結果を確認できます。

![Dry Run モード](demo/4_DryRunMode.gif)
